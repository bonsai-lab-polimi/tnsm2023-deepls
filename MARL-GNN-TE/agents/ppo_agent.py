...
class PPOAgent(object):
    ...
    def __init__(
        ...
        batch_size=25,
        epochs=3,
        last_training_sample=100,
        eval_period=50,
        max_evals=100,
        select_max_action=False,
        optimizer=tf.keras.optimizers.Adam(learning_rate=0.0003, beta_1=0.9, epsilon=0.00001),
    ...
    def train_and_evaluate(self):
        ...
        while not (self.env.num_sample == self.last_training_sample and self.change_sample == True):
            ...
            tf_logs.training_episode_logs(
                self.writer, self.env, training_episode, states, rewards, losses, actor_losses, critic_losses
            )
    ...
    def generate_eval_env(self):
        ...
        for eval_env_type in self.eval_env_type:
            self.eval_envs[eval_env_type] = Environment(
                env_type=eval_env_type, traffic_profile=self.env.traffic_profile, routing=self.env.routing
            )
    ...
    def update_eval_actor_critic_functions(self):
        for eval_env_type in self.eval_env_type:
            for w_model, w_eval_actor in zip(
                self.actor.trainable_variables, self.eval_actor[eval_env_type].trainable_variables
            ):
                w_eval_actor.assign(w_model)
            for w_model, w_eval_critic in zip(
                self.critic.trainable_variables, self.eval_critic[eval_env_type].trainable_variables
            ):
    ...
    def evaluation(self):
        ...
        if self.env.link_traffic_to_states:
            ...
            if self.only_eval:
                self.write_eval_results(self.eval_episode, np.min(max_link_utilization))
    ...
    def load_model(self, actor_model, critic_model):
        for w_model, w_actor in zip(actor_model, self.actor.trainable_variables):
            w_actor.assign(w_model)
        for w_model, w_critic in zip(critic_model, self.critic.trainable_variables):
            w_critic.assign(w_model)

    def load_saved_model(self, model_dir, only_eval):
        model = keras.models.load_model(model_dir + "/actor")
        for w_model, w_actor in zip(model.trainable_variables, self.actor.trainable_variables):
            w_actor.assign(w_model)
        if not only_eval:
            model = keras.models.load_model(model_dir + "/critic")
            for w_model, w_critic in zip(model.trainable_variables, self.critic.trainable_variables):
                w_critic.assign(w_model)
    ...
    def set_experiment_identifier(self, only_eval):
        ...
        if mode == "training":
            ...
            self.experiment_identifier = os.path.join(
                mode, eval_env_folder, env_folder, agent_folder, function_folder, episode
            )
