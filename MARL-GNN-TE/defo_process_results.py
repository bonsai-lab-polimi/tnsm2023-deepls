                ...
                for mp in mp_path:
                    ...
                    if len(end_path_info_list) == 0:
                        for sub_path_info in sub_path_info_lst:
                            end_path_info_list.append(
                                {"path": sub_path_info["path"][:-1], "proportion": sub_path_info["proportion"]}
                            )
                        ...
                        for path_info in end_path_info_list:
                            for sub_path_info in sub_path_info_lst:
                                ...
                                aux_end_path_list.append(
                                    {"path": new_path, "proportion": path_info["proportion"] * sub_path_info["proportion"]}
                                )
...
if __name__ == "__main__":
    network = "NSFNet"
    traffic = "uniform"
    defo_dir = f"MARL-GNN-TE/datasets/{network}/{traffic}/DEFO/"
    init_seed = 100
    final_seed = 199

    ecmp, defo = [], []
    graph_file = f"MARL-GNN-TE/datasets/{network}/{traffic}/capacities/graph.txt"
    for i in range(init_seed, final_seed + 1):
        results_file = defo_dir + "results-" + str(i)
        tm_file = f"MARL-GNN-TE/datasets/{network}/{traffic}/TM/TM-{i}"
    ...
    np.savetxt(f"MARL-GNN-TE/defo_results/{network}-{traffic}-ecmp.txt", np.array(ecmp), delimiter=",")
    np.savetxt(f"MARL-GNN-TE/defo_results/{network}-{traffic}-defo.txt", np.array(defo), delimiter=",")
